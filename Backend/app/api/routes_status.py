...
async def get_status(job_id: str, current_user: UserORM = Depends(get_current_user)):
    """
    DB is the source of truth for polled status — survives page refresh and
    worker restarts. job_store (in-memory) remains the live-push mechanism
    for the /stream SSE endpoint, not the source of truth here.
    """
    try:
        async with AsyncSessionLocal() as session:
            ...
            completed_pages = result.completed_pages or 0
            ...
            return JobStatusResponse(
                ...
                processed_files=completed_pages + failed_pages,
...
async def stream_job(job_id: str, token: str = Query(...)):
    ...
    if live.get("status") in ("done", "completed_with_errors", "error"):
        data = json.dumps({
